...
void Movie :: Show_details(){
    ...
    cout<<"The estimated budget is: "<<this->budget<<" euros"<<"\n";
...
Movie& Movie :: operator = (const Movie &m){  /// are sens sa fie folosit doar daca obiectul care face apelul a fost initializat cu Movie m(y);
    if(this != &m){
        ...
        this->cast = new string[m.n_cast];
        for(int i=0;i<m.n_cast;i++)
            cast[i] = m.cast[i];

        this->platforms = new char[m.n_platforms];
        for(int i=0;i<m.n_platforms;i++)
            platforms[i] = m.platforms[i];

        this->marks = new float[m.n_marks];
...
bool operator< (const Movie &m1, const Movie &m2){
    if(m1.views < m2.views) return false; /// l am facut invers deoarece vreau sa le sortez escrescator
    return true;
}
